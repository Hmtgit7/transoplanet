...
const TranslationForm = () => {
  ...
  const handleTranslation = async () => {
    if (!text) return;
    try {
      const response = await axios.post('https://transoplanet-backend.onrender.com', { text });
  ...
  const handleTextToSpeech = async () => {
    if (!text) return;
    try {
      const response = await axios.post('https://transoplanet-backend.onrender.com', { text: translatedText });
